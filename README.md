...
| `assignedAppIdsMap` | object: `{ [applicationId]: boolean }`          | Map of applications considered "Assigned" (e.g. already have capabilities in the persisted role). Drives the read-only Status column and the Status filter; never affected by checkbox interactions. If omitted, the Status column falls back to a snapshot of `checkedAppIdsMap` taken when the modal opens (checked === Assigned for that session), matching this plugin's behavior prior to the Status column's introduction. |          |
